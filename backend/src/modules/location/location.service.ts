...
interface PostcodesIoErrorResponse {
...
export class LocationService {
  ...
  async getCoordinates(address: string): Promise<Coordinates> {
    ...
    return this.cache.getOrSet(cacheKey, GEOCODE_CACHE_TTL_MS, () =>
      this.resolveCoordinates(trimmed),
    );
  }
  ...
  private async lookupPostcode(postcode: string): Promise<Coordinates | null> {
    try {
      const response = await axios.get<PostcodesIoResponse>(
        `${this.POSTCODES_IO_URL}/${encodeURIComponent(postcode)}`,
      );
      ...
      const terminated = axios.isAxiosError<PostcodesIoErrorResponse>(error)
        ? error.response?.data?.terminated
        : undefined;
  ...
  private async lookupCityName(address: string): Promise<Coordinates | null> {
    try {
      const response = await axios.get<PhotonResponse>(this.PHOTON_API_URL, {
        params: {
          q: address,
          limit: 1,
        },
